# gm_sorting_array.py: coded by Kinya MIURA 231229
# ---------------------------------------------------------
print('*** sorting: array ***')
# ---------------------------------------------------------
import numpy as np
...
ary2d = array([[0, 5, 7],
       [2, 8, 4],
       [6, 1, 3]])
np.sort(ary2d, axis=0) = array([[0, 1, 3],
       [2, 5, 4],
       [6, 8, 7]])
np.sort(ary2d, axis=0)[::-1] = array([[6, 8, 7],
       [2, 5, 4],
       [0, 1, 3]])
np.sort(ary2d, axis=1) = array([[0, 5, 7],
       [2, 4, 8],
       [1, 3, 6]])
np.sort(ary2d, axis=1)[::,::-1] = array([[7, 5, 0],
       [8, 4, 2],
       [6, 3, 1]]) 

np.argsort(ary2d, axis=0) = array([[0, 2, 2],
       [1, 0, 1],
       [2, 1, 0]], dtype=int64)
np.argsort(ary2d, axis=0)[::-1] = array([[2, 1, 0],
       [1, 0, 1],
       [0, 2, 2]], dtype=int64)
np.argsort(ary2d, axis=1) = array([[0, 1, 2],
       [0, 2, 1],
       [1, 2, 0]], dtype=int64)
np.argsort(ary2d, axis=1)[::,::-1] = array([[2, 1, 0],
       [1, 2, 0],
       [0, 2, 1]], dtype=int64)
'''
